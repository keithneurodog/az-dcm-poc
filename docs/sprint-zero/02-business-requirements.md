...
| ID | Requirement | Priority | JIRA Ref | Acceptance Criteria |
|----|-------------|----------|----------|---------------------|
| FR-COL-020 | The system shall support editing of draft collections by the DCM | Critical | -- | Given a draft collection, when the DCM edits it, then all fields are modifiable and changes are tracked |
| FR-COL-021 | The system shall enforce a collection lifecycle: DRAFT > SUBMITTED > IN REVIEW > APPROVED > ACTIVE > ARCHIVED | Critical | -- | Given a collection, when its status changes, then it must follow the defined lifecycle transitions |
| FR-COL-022 | The system shall support adding and removing studies from active collections within approved OAC scope | High | -- | Given an active collection with approved OAC scope, when studies are added/removed within that scope, then no re-approval is required |
| FR-COL-023 | The system shall require new approval when modifications fall outside the approved OAC or AOT scope | High | -- | Given a modification outside approved scope, when submitted, then the system routes the change for re-approval |
| FR-COL-024 | The system shall support collection archival with reason recording | Medium | -- | Given an active collection, when archived by a DCM, then the archive reason is recorded and all access is revoked with notification |
...
| **AOT** | Agreement of Terms - Formal document defining who can access what data under what terms |
