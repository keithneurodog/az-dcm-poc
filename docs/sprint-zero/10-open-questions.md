...
| Q-DATA-016 | How should AoT terms map to Immuta policy conditions? Is there a standard mapping or custom? **Partially Resolved:** The Immuta data model reveals: (a) two authorization tracks (IDA/AdHoc), (b) criteria-based User Profiles over User_Tags, (c) partition-based row security, (d) dual AI/ML flags, (e) intent taxonomy. Remaining: which of these does Collectoid create vs DPO manage? | P1 | 04-integration-map.md | Affects policy creation logic | Engineering + DPO | Partially Answered | |
...
| Q-DATA-023 | Does Collectoid own Immuta User Profile creation, or does Immuta maintain profiles independently? How does Collectoid specify which profiles get access? | P1 | Immuta data model analysis | Affects provisioning API design | Engineering + DPO | Open | |
| Q-DATA-024 | How should the dual AI/ML flags be transmitted to Immuta — as Data_Access_Intent columns, policy conditions, or separate authorisation attributes? | P1 | Immuta data model analysis | Affects AOT-to-Immuta mapping | Engineering + DPO | Open | |
| Q-DATA-025 | Who maintains User_Tag accuracy across 4 source systems (Manual, NPA, Workday, Cornerstone)? What is the refresh cadence? | P2 | Immuta data model analysis | Affects pre-provisioning validation | DPO + Identity Services | Open | |
| Q-DATA-026 | Should review cycle tracking be per-intent (new `collection_intents` table) or per-collection (current `review_date` on AoT)? | P2 | Immuta data model analysis | Affects data model and review workflow | Product Owner + Engineering | Open | |
| Q-DATA-027 | What Immuta mechanism handles column-level data masking (e.g., redacting PII columns)? Is masking configured as a policy attribute, a separate table, or an Immuta-native feature? Does Collectoid specify masking rules or is this DPO-managed? (Flagged as "Masking to be added" in PBAC metadata diagram.) | P1 | PBAC metadata diagram | Affects whether Collectoid needs a masking provisioning path in addition to the current 4-table model | Immuta Team / DPO | Open | |
| Q-DATA-028 | What monitoring and reporting capabilities does Immuta expose for policy enforcement verification? Can Collectoid query a status/health API to confirm provisioned policies are actively enforced at Starburst/Ranger? What reconciliation mechanisms exist? | P2 | PBAC metadata diagram | Affects compliance monitoring design and operational dashboard requirements | Immuta Team / Engineering | Open | |
