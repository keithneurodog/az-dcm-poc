...
const variationLoaders: Record<string, () => Promise<RouteVariations>> = {
  "/collectoid-v2/dcm/create": async () => {
    const mod = await import("@/app/collectoid-v2/(app)/dcm/create/_variations")
    return {
      variations: mod.variations,
      defaultVariation: mod.defaultVariation,
    }
  },
  "/collectoid-v2/collections": async () => {
    const mod = await import("@/app/collectoid-v2/(app)/collections/_variations")
    return {
      variations: mod.variations,
      defaultVariation: mod.defaultVariation,
    }
