...
const variationLoaders: Record<string, () => Promise<RouteVariations>> = {
  "/collectoid-v2/delivery-demo/dcm/create": async () => {
    const mod = await import("@/app/collectoid-v2/delivery-demo/dcm/create/_variations")
    return {
      variations: mod.variations,
      defaultVariation: mod.defaultVariation,
    }
  },
  "/collectoid-v2/delivery-demo/collections": async () => {
    const mod = await import("@/app/collectoid-v2/delivery-demo/collections/_variations")
    return {
      variations: mod.variations,
      defaultVariation: mod.defaultVariation,
    }
